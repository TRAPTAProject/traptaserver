...
QVariant ArcherTableModel::dataForDecorationRole(Archer *archer, int col) const {
    switch (col) {
        case TrispotColumn: {
            if (archer->trispot()) return QVariant(QIcon(":/images/checkbox.png"));
            break;
        }
        case TeammateColumn: {
            if (archer->teammate()) return QVariant(QIcon(":/images/checkbox.png"));
            break;
        }
        case Round1Column: {
            if (archer->warned(0)) return QVariant(QIcon(":/images/warning-icon.png"));
            break;
        }
        case Round2Column: {
            if (archer->warned(1)) return QVariant(QIcon(":/images/warning-icon.png"));
            break;
        }
        case Round3Column: {
            if (archer->warned(2)) return QVariant(QIcon(":/images/warning-icon.png"));
            break;
        }
        case Round4Column: {
            if (archer->warned(3)) return QVariant(QIcon(":/images/warning-icon.png"));
            break;
        }
    }
    return QVariant();
