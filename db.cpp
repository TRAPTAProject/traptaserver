...
DB::DB(const QString& dbfile, QObject *parent) : QObject(parent),
     _open(false)
{
    // Create db
...
int DB::insertMatchTab(const QString& roundName) {
...
bool DB::setRuleMap(const QList<QStringList>& list) {
...
QMap<int, QString> DB::matchTabMap() {
...
bool DB::deleteMatchById(int matchId) {
...
bool DB::deleteMatchCateg(int roundId, const QString& categ) {
    if (!_db.transaction()) {
        qWarning() << "Could not start transaction";
    }
...
bool DB::deleteMatchTab(int roundId) {
    if (!_db.transaction()) {
        qWarning() << "Could not start transaction";
    }
...
QList<int> DB::insertMatchList(QList<Match*> matchList) {
    QList<int> idList;
    if (!_db.transaction()) {
        qWarning() << "Could not start transaction";
    }
...
Archer* DB::createArcher(int id, int shootId) {
...
bool DB::updateMatch(int matchId,
                 int position,
                 bool trispot,
                 int tieBreak,
                 int mode,
...
bool DB::clearPointsByShootId(int shootId) {
    if (!_db.transaction()) {
        qWarning() << "Could not start transaction";
    }
...
bool DB::clearPointsByArcherId(int id, int roundIndex) {
     QSqlQuery query(_db);
...
bool DB::deleteArcherById(int id) {
    if (!_db.transaction()) {
        qWarning() << "Could not start transaction";
    }
    QSqlQuery query(_db);
...
bool DB::deleteArcherByShootId(int shootId) {
    if (!_db.transaction()) {
        qWarning() << "Could not start transaction";
    }
    QString sql = QString("SELECT * FROM `archer` WHERE `shootId`=%0").arg(shootId);
...
bool DB::updateHeatScoreCard(int archerId, int heatIndex, QList<Volley> volleyList) {
...
// this deletes and insert a new volley list
bool DB::updateMatchScoreCard(int matchId, QList< QList<Volley> > tabList) {
...
bool DB::loadFromStringList(const QStringList& stringlist) {
